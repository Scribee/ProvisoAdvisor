...
GET_SKILLS = 'SELECT skills.ID, skills.Name, COUNT(skills.ID) AS Count FROM students, taken, classes, teaches, skills WHERE students.ID=1234 AND students.ID=taken.ID AND taken.Class=classes.Class AND classes.Class=teaches.Class AND teaches.SkillID=skills.ID GROUP BY skills.Name'
